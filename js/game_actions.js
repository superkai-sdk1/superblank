(function($){
    ...
    // Функция для проверки правильности распределения ролей
    ...
    window.setFall = function(delta, value) {
        if ((value >= 0) && (value <= 4)) {
            ...
            if (input && view) {
                input.value = value; // Обновляем значение скрытого поля
                view.textContent = value; // Обновляем текст ячейки
                view.className = `fall_${value}`; // Обновляем класс для стилей
            }
        }
    };

    // Обработчики для кнопок "Победа Мафии" и "Победа Мирных"
    document.getElementById('mafia').addEventListener('click', function() {
        if (checkValues() === 0) {
            // Логика для победы Мафии
        } else {
            this.checked = false;
        }
    });
    ...
    window.setFall = function(delta, value) {
        if ((value >= 0) && (value <= 4)) {
            ...
            if (input && view) {
                input.value = value; // Обновляем значение скрытого поля
                view.textContent = value; // Заменяем текст ячейки (используем =, а не +=)
                view.className = `fall_${value}`; // Обновляем класс для стилей
    ...
    function applyFallPenalty() {
        for (let i = 0; i < 10; i++) {
            ...
            if (fallInput && addPoints) {
                ...
                // Если у игрока 4 фола, добавляем штраф
                if (fallValue === 4) {
                    const currentPoints = parseFloat(addPoints.value) || 0;
                    const penalty = -0.5;
                    addPoints.value = (currentPoints + penalty).toFixed(2);
